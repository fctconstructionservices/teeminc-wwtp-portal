...
const Nav = {
    ...
    render(pageId, tab) {
        ...
        // The company is whatever config says it is — this same bar is
        // rendered for more than one company, so a name or an initial
        // written in here shows the wrong one on the other's system.
        const brand = (typeof CONFIG !== 'undefined' && CONFIG.company && CONFIG.company.short) || '';
        const initial = brand.charAt(0) || '';
        const logo = url
            ? `<img src="${e(url)}" alt="${e(brand)}" onerror="this.replaceWith(document.createTextNode('" + e(initial) + "'))" />`
            : e(initial);
    ...
    paintBranding() {
        ...
        const apply = (url) => {
            // The nav bar builds its own mark synchronously from the
            // cached template, which on a cold load has not arrived yet —
            // so it renders the lettered box and would keep it until the
            // next navigation. Fill it in here once the logo is known.
