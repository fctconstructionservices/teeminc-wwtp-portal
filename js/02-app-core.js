// ================================================================
        ...
        const App = {
            ...
            navigate(page) {
                ...
                // Update user badges (requester, liquidator, etc.)
            ...
            canSubmit() {
                return true; // all logged-in users can submit
            },

            // Update any badge that shows the current user's name
            updateUserBadges() {
                const user = this.getUser();
                if (!user) return;
                const name = user.name || 'User';
                // Update requester badges (e.g., "Requesting as: <strong>Name</strong>")
                document.querySelectorAll('.requester-badge').forEach(el => {
                    const strong = el.querySelector('strong');
                    if (strong) {
                        strong.textContent = name;
                    } else {
                        // If no strong, attempt to set text
        ...
        const SOWBreakdownModal = {
            ...
            renderBreakdown(sowId, data) {
                // Filter data by sowId — support both old and new structure
                let materials = [],
                    equipment = [],
                    labor = [],
                    indirect = [];
        ...
        async function handleLogin(e) {
            ...
            let user;
            try {
                // Server-side check laban sa Users sheet (hindi na hardcoded sa JS)
